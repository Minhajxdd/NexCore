...
export async function addCartProducts(req, res) {
  ...
  const quantity =
    quantityString !== undefined ? Number(quantityString) : undefined;
  ...
  if (!productId) {
    ...
    return res
      .status(404)
      .json({ status: "failed", message: "productId not found" });
  }
  ...
  try {
    ...
    console.log(
      `error while checkign maximum stock for one user: ${err.message}`
    );
  }
  ...
  try {
    ...
    const stockCount = await productModel.findById(productId, {
      _id: 0,
      stock: 1,
    });
    ...
    if (userData.cartId) {
      ...
      await userModel.findByIdAndUpdate(userId, { cartId: cartData._id });
...
export async function productQuantityInc(req, res) {
  ...
  try {
    ...
    if (!updateResult) {
      return res
        .status(400)
        .json({ status: "failed", message: "Quantity could not be increased" });
    }
...
export async function productQuantityDec(req, res) {
  ...
  try {
    ...
    const updateResult = await cartModel.findOneAndUpdate(
      ...
      {
        arrayFilters: [
          { "elem.product_id": productId, "elem.quantity": { $gt: 1 } },
        ],
        new: true,
      }
    );
    ...
    if (!updateResult) {
      return res.status(400).json({
        status: "failed",
        message: "Quantity could not be decremented",
      });
    }
    const totalPrice = await cartTotalUpdate(cartId);
...
async function cartTotalUpdate(cartId) {
  try {
    ...
    console.log(
      `Error while updating total price on cartController ${err.message}`
    );
    return res
      .status(500)
      .json({ status: "failed", message: "Internal server error" });
...
export async function deleteCartProduct(req, res) {
  ...
  if (!cartId) {
    return res
      .status(404)
      .json({ status: "failed", message: "cart id not found" });
  }

  if (!productId) {
    return res
      .status(404)
      .json({ status: "failed", message: "product id not found" });
  }

  try {
    const cart = await cartModel.findById(cartId, { _id: 0, items: 1 });

    const SubPrice = cart.items.find((item) =>
      item.product_id.equals(productId)
    ).price;
    ...
    console.log(
      `Error which deleting cart products on cartController : ${err.message}`
    );
  }
}
